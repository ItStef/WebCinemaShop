...
export class MovieService { 
    ...
    static getMovieById(id: string): MovieModel | null {
        return this.getAllMovies().find(movie => movie.id === id) || null;
    }
    ...
    static getMovieDescription(title: string): string {
        const movie = this.getAllMovies().find(m => m.title === title);
        return movie ? movie.description : 'No description available';
    }
    ...
    static updateMovieRating(movieId: string, newRating: number): boolean {
        try {
            ...
            movie.averageRating = Math.round(newRating * 10) / 10;
    ...
    static getScreeningById(movie: MovieModel, screeningId: string): ScreeningModel | null {
        return movie.screenings.find(screening => screening.id === screeningId) || null;
    }
}